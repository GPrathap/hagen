...
void LidarProcessSubnode::onPointCloud(const sensor_msgs::PointCloud2& cloud) {
  ...
  // try {
  //   m_tfListener.lookupTransform(world_frame_id, cloud.header.frame_id, ros::Time(0), sensor_to_world_tf);
  ...
  if((int)internal_cloud_ptr_->size() == number_point_clouds_to_be_merged){
      ...
      std::string filename = "/home/geesara/catkin_ws/src/test/hagen_filter.csv";
      ...
      point_cloud_non_ground_plane_cloud.header.frame_id = "velodyne";
...
void LidarProcessSubnode::separate_ground_and_non_ground(Cloud::Ptr& cloud_ptr_current_ptr
  , pcl::PointCloud<PCLPoint> pc){
          ...
          depth_ground_remover->options.kernel_size = kernel_size;
